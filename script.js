...
async async function startTask(task) {
  ...
  try {
    ...
    const oldBox = document.getElementById("taskProofBox");
    if (oldBox) {
      oldBox.remove();
    }
    ...
    // Submit
    document.getElementById("taskSubmitBtn").onclick = async () => {
      const proofInput =
      ...
      if (!proof) {
        alert("Proof না দিলে Task Submit করা যাবে না।");
        return;
      }
      ...
      try {
        ...
        if (error) {
          ...
          if (error.code === "23505") {
            alert("আপনি এই Task ইতিমধ্যে Submit করেছেন।");
          } else {
            alert(
